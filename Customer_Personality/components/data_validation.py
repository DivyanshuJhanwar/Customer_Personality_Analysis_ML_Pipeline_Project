...
class DataValidation:
    def __init__(self, app_config = AppConfiguration()):
        try:
            self.data_validation_config= app_config.get_data_validation_config()  # calling get_data_validation_config() function from configuration file
    ...
    def preprocess_data(self):
        try:
            ...
            df = pd.read_csv(self.data_validation_config.marketing_campaign_csv_file,  sep='\t') # using sep='\t' because in between the data we have comma's
            logging.info(f" Shape of the data file: {df.shape}")
            #Droping null
            df.dropna(inplace=True)  # dropna is used for dropping complete null data
           ...
           # Converting to datetime with the correct date format
